...
print "Promedio de acciones mediante exploracion:\n"
for j in xrange(15):
    Q = np.zeros((height * width, num_actions))
    promedio_exploracion = 0

    # Episodes for exploration
    for i in xrange(100):
        state = getRndState()
        while state != final_state:
            action = getRndAction(state)
            promedio_exploracion += 1
            ...
            state = new_state
    print Q
    print promedio_exploracion/100
print "----------------------------------------------"

print "Promedio de acciones mediante explotacion:\n"
for j in xrange(15):
    Q = np.zeros((height * width, num_actions))

    promedio_explotacion = 0
    # Episodes for exploitation
    for i in xrange(100):
        state = getRndState()
        while state != final_state:
            if max(Q[state])!= 0:
                indice = np.argmax(Q[state]) #Indice
                action = actions_list_inv[indice]
            else:
                action = getRndAction(state)

            promedio_explotacion += 1
            y = getStateCoord(state)[0] + actions_vectors[action][0]
            x = getStateCoord(state)[1] + actions_vectors[action][1]
            new_state = getState(y, x)
            qlearning(state, actions_list[action], new_state)
    ...
    print Q
    print promedio_explotacion/100



# I.E PERCENTAGE %90, %80, %70, %60...
print "-------------------------------------------------------"

print "Promedio de acciones mediante E-Greedy:\n"
for j in xrange(15):
    Q = np.zeros((height * width, num_actions))
    prm_e_greedy = 0
    # Episodes for E-Greedy
    for i in xrange(100):
        state = getRndState()
        while state != final_state:
            if max(Q[state]) != 0:
